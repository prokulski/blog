...
for(i in nr_glosowan) {
  ...
  df <- bind_cols(df, Rtsne(df[, 2:ncol(df)], check_duplicates = FALSE)$Y %>%
                    as_tibble() %>%
                    set_names(c("V1", "V2"))) %>%
    select(osoba, V1, V2) %>%
    mutate(id_glosowania = i)

  tsne_total <- bind_rows(tsne_total, df)
...
p <- tsne_total %>%
  ...
  #             select(id_glosowania, data_posiedzenia),
  #           by = "id_glosowania") %>%
  # group_by(osoba, data_posiedzenia) %>%
  # summarise_at(c("V1", "V2"), mean, na.rm = TRUE) %>%
  # ungroup() %>%
  ggplot() +
  geom_point(aes(V1, V2, color = osoba))



library(gganimate)

anim <- p + transition_time(id_glosowania) + shadow_wake(wake_length = 0.1, alpha = FALSE)
